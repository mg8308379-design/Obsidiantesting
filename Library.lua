...
local function OpenTabContext()
    ...
    local function OpenRenamePopup()
        ...
        local currentBtnPos = TabButton.AbsolutePosition
        local currentBtnSize = TabButton.AbsoluteSize

        RenamePopup = New("Frame", {
            BackgroundColor3 = Color3.fromRGB(18, 19, 22),
            Position = UDim2.fromOffset(
                currentBtnPos.X + currentBtnSize.X + 4,
                currentBtnPos.Y
            ),
            Size = UDim2.fromOffset(190, 95),
            ZIndex = 910,
            Parent = ScreenGui,
        })
        ...
        New("UICorner", { CornerRadius = UDim.new(1, 0), Parent = CancelX })
        ...
        -- Follow tab button if UI is dragged
        local posConnection = TabButton:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
            local p = TabButton.AbsolutePosition
            local s = TabButton.AbsoluteSize
            RenamePopup.Position = UDim2.fromOffset(p.X + s.X + 4, p.Y)
        end)
    ...
    local function CreateMenuBtn(text, color, callback)
        local btn = New("TextButton", {
            ...
            AutoButtonColor = false,
            Text = text,
            TextColor3 = color or Color3.fromRGB(185, 187, 190),
            TextSize = 12,
            TextXAlignment = Enum.TextXAlignment.Left,
            ZIndex = 901,
            Parent = TabContextMenu,
        ...
        btn.MouseLeave:Connect(function()
            ...
            btn.TextColor3 = color or Color3.fromRGB(185, 187, 190)
        end)
    ...
    CreateMenuBtn("Reset Name", nil, function()
        TabLabel.Text = Name
    ...
    -- Follow tab button
    local ctxConnection = TabButton:GetPropertyChangedSignal("AbsolutePosition"):Connect(function()
        local p = TabButton.AbsolutePosition
        local s = TabButton.AbsoluteSize
        TabContextMenu.Position = UDim2.fromOffset(p.X + s.X + 4, p.Y)
    end)

    local originalClose = CloseTabContext
    CloseTabContext = function()
        ctxConnection:Disconnect()
        CloseRenamePopup()
        if TabContextMenu then
            TabContextMenu:Destroy()
            TabContextMenu = nil
        end
        CloseTabContext = originalClose
    end
end
